    ...
    def crop_n_parts(self,n):
        ...
        for x in range(1,n+1):
            for y in range(1,n+1):
                if x==n and y==n:
                    result=self.image_crop(int((x-1)*H),int(self.height)-1,int((y-1)*W),int(self.width)-1)
                elif x==n :
                    result=self.image_crop(int((x-1)*H),int(self.height)-1,int((y-1)*W),int(y*W))
                elif y==n :
                    result=self.image_crop(int((x-1)*H),int((x)*H),int((y-1)*W),int(self.width)-1)
                else:
                    result=self.image_crop(int((x-1)*H),int((x)*H),int((y-1)*W),int(y*W))
                cv.imwrite('saved_patches/' + 'tile' + str(x) + '_' + str(y) + '.jpg', result)
